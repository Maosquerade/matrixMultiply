...
__global__ void matrixMul(float *a, float *b, float *c, int n) {
    __shared__ float sa[32][33]; // A shared memory array

    float cr[32] = {0.f}; // C register
    int nDiv32 = n / 32;
    ...
    int tid = sRow * 32 + sCol;
    int aNext = (32 * blockIdx.y + sRow) * n + sCol2; // 这里怪怪的, 一个 block 读 16*32 的 a 的子矩阵，因为每行读两个所以乘2
    ...
    for (i = 0; i < nDiv32; i++) {
        ...
        for (j = 0; j < 32; j++) {
            ...
            if (blockIdx.x == 7 && blockIdx.y == 31 && threadIdx.x == 15 && threadIdx.y == 0 && i == 31 && j == 31) printf("1. threadIdx.x = %d, threadIdx.y = %d. cr = %f.\n", threadIdx.x, threadIdx.y, cr[0]);
        }
        // temp += 16;
        a += 32;
        __syncthreads();
    ...
    int cNext = 32 * blockIdx.y * n + 128 * blockIdx.x + tid;
    c += cNext;
    for (int k = 0; k < 32; k++) {
        c[0] = cr[k];
        c += n;
        if (blockIdx.x == 7 && blockIdx.y == 31 && threadIdx.x == 0 && threadIdx.y == 0) printf("I'm here too %d.\n", k);
    }
    if (blockIdx.x == 7 && blockIdx.y == 31 && threadIdx.x == 0 && threadIdx.y == 0) printf("C[0] = %f.\n", c[0]);
}
